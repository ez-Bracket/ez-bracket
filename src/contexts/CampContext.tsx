...
interface iCampConext {
    ...
    winnerPlayerCompetition: (idCamp: number, round:number, chave:number, winnerPlayer:string) => void
}
...
export const CampProvider = ({children}: iCampProvidertProps) => {
    ...
    const getCompetition = async (idUser: number) =>{
        try{
            ...
            setCamp(allGames.data.filter((element: iCamp)=> element.idUser === idUser))
        }catch(error){
            console.log(error)
        }
    }
    ...
    const winnerPlayerCompetition = async (idCamp: number, round:number, chave:number, winnerPlayer:string) => {
        try{
            const config = {
                headers: {
                    Authorization: `Bearer ${window.localStorage.getItem("@EZ:TOKEN")}`
            ...
            if(game.data.games[round-1].length >= chave){                
                ...
                if(!game.data.games[round-1].find((element: {winner: string}) => element.winner === "") && game.data.games[round-1].length > 1){
                    console.log(`Round ${round}° Acabou!`);
                
                    const createGames = [];
                    for(let index = 0;  index < game.data.games[round-1].length; index=index + 2){
                        createGames.push({
